...
import { c as cn, S as Spinner, i as isLoggedIn, r as removeAuthToken } from './BaseLayout_B-BXcELP.mjs';
...
function AICourseLessonChat(props) {
  ...
  const completeCourseAIChat = async (messages) => {
    ...
    const response = await fetch(
      `${"https://api.imoogleai.xyz"}/v1-follow-up-ai-course/${courseSlug}`,
      {
        method: "POST",
        headers: {
