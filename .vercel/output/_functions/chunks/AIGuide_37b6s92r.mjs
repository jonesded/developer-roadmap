...
import { q as queryClient, a as httpPost } from './query-client_B4F1D3EC.mjs';
...
import { i as isLoggedIn, u as useOutsideClick, c as cn, b as getUrlParams } from './BaseLayout_C3JOBLhD.mjs';
import { g as getAiGuideOptions, a as aiGuideSuggestionsOptions } from './ai-guide_-ZgsIGHA.mjs';
...
import { u as useChat } from './use-chat_BD5l5sXU.mjs';
import { a as RoadmapAIChatCard } from './UpdatePersonaModal_CmDp_pWg.mjs';
import { C as ChatHeaderButton } from './RoadmapFloatingChat_DYg2wr67.mjs';
import { s as showLoginPopup } from './popup_DWUkHIfQ.mjs';
import { b as billingDetailsOptions, u as useIsPaidUser } from './billing_BlRWCv4l.mjs';
  ...
  try {
    ...
    if (slug && isForce) {
      response = await fetch(
        `${"https://api.roadmap.sh"}/v1-regenerate-ai-guide/${slug}`,
        {
          method: "POST",
          headers: {
      ...
      response = await fetch(
        `${"https://api.roadmap.sh"}/v1-generate-ai-guide`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
...
function AIGuideChat(props) {
  ...
  const {
    ...
    endpoint: `${"https://api.roadmap.sh"}/v1-ai-guide-chat`,
    onError: (error) => {
      console.error(error);
    },
    data: {
