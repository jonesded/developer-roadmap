...
import { i as isLoggedIn, u as useOutsideClick, c as cn, b as getUrlParams } from './BaseLayout_B-BXcELP.mjs';
import { g as getAiGuideOptions, a as aiGuideSuggestionsOptions } from './ai-guide_DwS_b0Br.mjs';
...
import { b as billingDetailsOptions, u as useIsPaidUser } from './billing_5mov2PsZ.mjs';
  ...
  try {
    ...
    if (slug && isForce) {
      response = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-regenerate-ai-guide/${slug}`,
        {
          method: "POST",
          headers: {
      ...
      response = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-generate-ai-guide`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
...
function AIGuideChat(props) {
  ...
  const {
    ...
    endpoint: `${"https://api.imoogleai.xyz"}/v1-ai-guide-chat`,
    onError: (error) => {
      console.error(error);
    },
    data: {
