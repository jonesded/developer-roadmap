...
import { a as aiRoadmapOptions, g as generateAIRoadmap } from './ai-roadmap_5E8uzWVK.mjs';
import { u as useOutsideClick, c as cn, i as isLoggedIn, b as getUrlParams } from './BaseLayout_DfQviQZ5.mjs';
  ...
  const { mutate: editAIRoadmap, isPending: isEditingAIRoadmap } = useMutation(
    {
      ...
      onSuccess: (data) => {
        ...
        window.open(
          `${"https://draw.hnmdevs.com"}/${data?.roadmapId}`,
          "_blank"
        );
      }
    },
...
function AIRoadmapChat(props) {
  ...
  const {
    ...
    endpoint: `${"https://api.hnmdevs.com"}/v1-ai-roadmap-chat`,
    onError: (error) => {
      console.error(error);
