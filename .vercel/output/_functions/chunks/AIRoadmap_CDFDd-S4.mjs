...
import { a as aiRoadmapOptions, g as generateAIRoadmap } from './ai-roadmap_CH5_TijD.mjs';
import { u as useOutsideClick, c as cn, i as isLoggedIn, b as getUrlParams } from './BaseLayout_C3JOBLhD.mjs';
  ...
  const { mutate: editAIRoadmap, isPending: isEditingAIRoadmap } = useMutation(
    {
      ...
      onSuccess: (data) => {
        ...
        window.open(
          `${"https://draw.roadmap.sh"}/${data?.roadmapId}`,
          "_blank"
        );
      }
    },
...
function AIRoadmapChat(props) {
  ...
  const {
    ...
    endpoint: `${"https://api.roadmap.sh"}/v1-ai-roadmap-chat`,
    onError: (error) => {
      console.error(error);
