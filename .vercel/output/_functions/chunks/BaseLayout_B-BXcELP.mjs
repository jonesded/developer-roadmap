...
function GoogleButton(props) {
  ...
  useEffect(() => {
    ...
    httpGet(
      `${"https://api.imoogleai.xyz"}/v1-google-callback${window.location.search}&src=${urlToId(lastPageBeforeGoogle || getLastPath() || window.location.pathname)}`
  ...
  const handleClick = () => {
    ...
    setIsDisabled?.(true);
    httpGet(
      `${"https://api.imoogleai.xyz"}/v1-google-login`
    ).then(({ response, error: error2 }) => {
...
function LinkedInButton(props) {
  ...
  useEffect(() => {
    ...
    httpGet(
      `${"https://api.imoogleai.xyz"}/v1-linkedin-callback${window.location.search}&src=${urlToId(lastPageBeforeLinkedIn || getLastPath() || window.location.pathname)}`
    ).then(({ response, error: error2 }) => {
      if (!response?.token) {
        setError(error2?.message || "Something went wrong.");
        setIsLoading(false);
  ...
  const handleClick = () => {
    ...
    setIsDisabled?.(true);
    httpGet(
      `${"https://api.imoogleai.xyz"}/v1-linkedin-login`
    ).then(({ response, error: error2 }) => {
      if (!response?.loginUrl) {
        setError(error2?.message || "Something went wrong.");
        setIsLoading(false);
...
function EmailLoginForm(props) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    setIsDisabled?.(true);
    setError("");
    const { response, error: error2 } = await httpPost(`${"https://api.imoogleai.xyz"}/v1-login`, {
      email,
      password
    });
    if (response?.token) {
...
function EmailSignupForm(props) {
  ...
  const onSubmit = async (e) => {
    ...
    const { response, error: error2 } = await httpPost(
      `${"https://api.imoogleai.xyz"}/v1-register`,
      {
        email,
        password,
        name,
...
function GitHubButton(props) {
  ...
  useEffect(() => {
    ...
    httpGet(
      `${"https://api.imoogleai.xyz"}/v1-github-callback${window.location.search}&src=${urlToId(lastPageBeforeGithub || getLastPath() || window.location.pathname)}`
    ).then(({ response, error: error2 }) => {
      if (!response?.token) {
  ...
  const handleClick = async () => {
    ...
    const { response, error: error2 } = await httpGet(
      `${"https://api.imoogleai.xyz"}/v1-github-login`
    );
...
function PageVisit(props) {
  ...
  useEffect(() => {
    ...
    httpPost(`${"https://api.imoogleai.xyz"}/v1-visit`, {
