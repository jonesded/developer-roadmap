...
import { M as Modal, c as cn, h as httpPost } from './BaseLayout_B-BXcELP.mjs';
...
function CreateRoadmapModal(props) {
  ...
  async function handleSubmit(e, redirect = true) {
    ...
    const { response, error } = await httpPost(
      `${"https://api.imoogleai.xyz"}/v1-create-roadmap`,
      {
        title,
        description,
        ...teamId && {
    ...
    if (redirect) {
      window.location.href = `${"https://draw.imoogleai.xyz"}/${response?._id}`;
      return;
    }
    if (onCreated) {
      onCreated(response);
