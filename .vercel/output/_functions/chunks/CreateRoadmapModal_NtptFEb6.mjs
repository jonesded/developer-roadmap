...
import { M as Modal, c as cn, h as httpPost } from './BaseLayout_DfQviQZ5.mjs';
...
function CreateRoadmapModal(props) {
  ...
  async function handleSubmit(e, redirect = true) {
    ...
    const { response, error } = await httpPost(
      `${"https://api.hnmdevs.com"}/v1-create-roadmap`,
      {
        title,
        description,
        ...teamId && {
    ...
    if (redirect) {
      window.location.href = `${"https://draw.hnmdevs.com"}/${response?._id}`;
      return;
    }
    if (onCreated) {
      onCreated(response);
