...
import { c as cn, S as Spinner, e as httpGet, M as Modal, b as getUrlParams, p as pageProgressMessage, u as useOutsideClick, l as useKeydown, h as httpPost, g as getUser, R as RoadmapIcon, w as httpPut } from './BaseLayout_C3JOBLhD.mjs';
...
import { C as CreateRoadmapButton, F as FeaturedGuideList, a as FeaturedVideoList } from './CreateRoadmapButton_DIfKMB97.mjs';
import { C as CheckIcon } from './CheckIcon_B3c1RSNy.mjs';
import { g as getRelativeTimeString } from './date_iLNlXR7B.mjs';
import { M as MarkFavorite } from './MarkFavorite_5WMrRtGm.mjs';
import { C as CreateRoadmapModal } from './CreateRoadmapModal_CiC6izg_.mjs';
import { u as useIsPaidUser } from './billing_BlRWCv4l.mjs';
import { U as UpgradeAccountModal } from './UpgradeAccountModal_CxrAT9TD.mjs';
import 'js-cookie';
import { P as Pagination } from './Pagination_BY4mUo2a.mjs';
...
function PersonalDashboard(props) {
  ...
  async function loadProgress() {
    const { response: progressList, error } = await httpGet(
      `${"https://api.roadmap.sh"}/v1-user-dashboard`
    );
...
function TeamActivityPage(props) {
  ...
  const getTeamProgress = async (currPage2 = 1) => {
    const { response, error } = await httpGet(
      `${"https://api.roadmap.sh"}/v1-get-team-activity/${teamId}`,
      {
        currPage: currPage2
      }
    );
...
function ResourceProgressActions(props) {
  ...
  async function clearProgress() {
    ...
    const { error, response } = await httpPost(
      `${"https://api.roadmap.sh"}/v1-clear-resource-progress`,
      {
        resourceId,
        resourceType
      }
...
function DashboardTeamRoadmaps(props) {
  ...
  async function onAdd(roadmapId, shouldCopyContent = false) {
    ...
    const { error, response } = await httpPut(
      `${"https://api.roadmap.sh"}/v1-update-team-resource-config/${teamId}`,
      {
        teamId,
  ...
  async function deleteResource(roadmapId) {
    ...
    const { error, response } = await httpPut(
      `${"https://api.roadmap.sh"}/v1-delete-team-resource-config/${teamId}`,
      {
        resourceId: roadmapId,
        resourceType: "roadmap"
      }
...
function InviteMemberPopup(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, error: error2 } = await httpPost(
      `${"https://api.roadmap.sh"}/v1-invite-member/${teamId}`,
...
function TeamDashboard(props) {
  ...
  async function getTeamProgress() {
    const { response, error } = await httpGet(
      `${"https://api.roadmap.sh"}/v1-get-team-progress/${teamId}`
    );
    if (error || !response) {
