...
import { c as cn, S as Spinner, e as httpGet, M as Modal, b as getUrlParams, p as pageProgressMessage, u as useOutsideClick, l as useKeydown, h as httpPost, g as getUser, R as RoadmapIcon, w as httpPut } from './BaseLayout_DfQviQZ5.mjs';
...
import { C as CreateRoadmapButton, F as FeaturedGuideList, a as FeaturedVideoList } from './CreateRoadmapButton_BXYVD-Ti.mjs';
import { C as CheckIcon } from './CheckIcon_B3c1RSNy.mjs';
import { g as getRelativeTimeString } from './date_iLNlXR7B.mjs';
import { M as MarkFavorite } from './MarkFavorite_C_pnbLz-.mjs';
import { C as CreateRoadmapModal } from './CreateRoadmapModal_NtptFEb6.mjs';
import { u as useIsPaidUser } from './billing_B5wE6dhK.mjs';
import { U as UpgradeAccountModal } from './UpgradeAccountModal_DhdxALHd.mjs';
import 'js-cookie';
import { P as Pagination } from './Pagination_DCvqQ5el.mjs';
...
function PersonalDashboard(props) {
  ...
  async function loadProgress() {
    const { response: progressList, error } = await httpGet(
      `${"https://api.hnmdevs.com"}/v1-user-dashboard`
    );
...
function TeamActivityPage(props) {
  ...
  const getTeamProgress = async (currPage2 = 1) => {
    const { response, error } = await httpGet(
      `${"https://api.hnmdevs.com"}/v1-get-team-activity/${teamId}`,
      {
        currPage: currPage2
      }
    );
...
function ResourceProgressActions(props) {
  ...
  async function clearProgress() {
    ...
    const { error, response } = await httpPost(
      `${"https://api.hnmdevs.com"}/v1-clear-resource-progress`,
      {
        resourceId,
        resourceType
      }
...
function DashboardTeamRoadmaps(props) {
  ...
  async function onAdd(roadmapId, shouldCopyContent = false) {
    ...
    const { error, response } = await httpPut(
      `${"https://api.hnmdevs.com"}/v1-update-team-resource-config/${teamId}`,
      {
        teamId,
  ...
  async function deleteResource(roadmapId) {
    ...
    const { error, response } = await httpPut(
      `${"https://api.hnmdevs.com"}/v1-delete-team-resource-config/${teamId}`,
      {
        resourceId: roadmapId,
        resourceType: "roadmap"
      }
...
function InviteMemberPopup(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, error: error2 } = await httpPost(
      `${"https://api.hnmdevs.com"}/v1-invite-member/${teamId}`,
...
function TeamDashboard(props) {
  ...
  async function getTeamProgress() {
    const { response, error } = await httpGet(
      `${"https://api.hnmdevs.com"}/v1-get-team-progress/${teamId}`
    );
    if (error || !response) {
...
function DashboardPage(props) {
  ...
  async function getAllTeams() {
    ...
    const { response, error } = await httpGet(
      `${"https://api.hnmdevs.com"}/v1-get-user-teams`
    );
    if (error || !response) {
      toast.error(error?.message || "Something went wrong");
      return;
