...
import { c as cn, u as useOutsideClick, i as isLoggedIn, r as removeAuthToken, l as useKeydown, M as Modal, b as getUrlParams, S as Spinner, B as GitHubIcon, C as parseUrl, e as httpGet, p as pageProgressMessage } from './BaseLayout_C3JOBLhD.mjs';
import { g as getTopicStatus, u as updateResourceProgress, a as renderTopicProgress, b as refreshProgressCounters, i as isTopicDone } from './resource-progress_Dhi8X1Qg.mjs';
...
import { r as roadmapTreeMappingOptions, u as userResourceProgressOptions } from './roadmap-tree_DhFfmgQ6.mjs';
import { q as queryClient } from './query-client_B4F1D3EC.mjs';
import { A as AIChatCard } from './AICourseLessonChat_D_-Heojr.mjs';
/* empty css                          */
  ...
  const completeAITutorChat = async (messages) => {
    try {
      ...
      const response = await fetch(
        `${"https://api.roadmap.sh"}/v1-topic-detail-chat`,
        {
          method: "POST",
          headers: {
...
async function fetchRoadmapPaidResources(roadmapId) {
  ...
  const { response, error } = await httpGet(
    `${"https://api.roadmap.sh"}/v1-list-roadmap-paid-resources/${roadmapId}`
  );
  if (!response || error) {
    console.error(error);
    return [];
...
function TopicDetail(props) {
  ...
  useLoadTopic(({ topicId: topicId2, resourceType: resourceType2, resourceId: resourceId2, isCustomResource: isCustomResource2 }) => {
    ...
    if (isCustomResource2) {
      topicUrl = `${"https://api.roadmap.sh"}/v1-get-node-content/${resourceId2}/${topicId2}${secret ? `?secret=${secret}` : ""}`;
    }
    httpGet(
      topicUrl,
      {},
