...
import { c as cn, u as useOutsideClick, i as isLoggedIn, r as removeAuthToken, l as useKeydown, M as Modal, b as getUrlParams, S as Spinner, B as GitHubIcon, C as parseUrl, e as httpGet, p as pageProgressMessage } from './BaseLayout_DfQviQZ5.mjs';
import { g as getTopicStatus, u as updateResourceProgress, a as renderTopicProgress, b as refreshProgressCounters, i as isTopicDone } from './resource-progress_JteJuO5t.mjs';
...
import { r as roadmapTreeMappingOptions, u as userResourceProgressOptions } from './roadmap-tree_SwLtcRIZ.mjs';
import { q as queryClient } from './query-client_BnT_580V.mjs';
import { A as AIChatCard } from './AICourseLessonChat_B-0UsG6B.mjs';
/* empty css                          */
  ...
  const completeAITutorChat = async (messages) => {
    try {
      ...
      const response = await fetch(
        `${"https://api.hnmdevs.com"}/v1-topic-detail-chat`,
        {
          method: "POST",
          headers: {
...
async function fetchRoadmapPaidResources(roadmapId) {
  ...
  const { response, error } = await httpGet(
    `${"https://api.hnmdevs.com"}/v1-list-roadmap-paid-resources/${roadmapId}`
  );
  if (!response || error) {
    console.error(error);
    return [];
...
function TopicDetail(props) {
  ...
  useLoadTopic(({ topicId: topicId2, resourceType: resourceType2, resourceId: resourceId2, isCustomResource: isCustomResource2 }) => {
    ...
    if (isCustomResource2) {
      topicUrl = `${"https://api.hnmdevs.com"}/v1-get-node-content/${resourceId2}/${topicId2}${secret ? `?secret=${secret}` : ""}`;
    }
    httpGet(
      topicUrl,
      {},
