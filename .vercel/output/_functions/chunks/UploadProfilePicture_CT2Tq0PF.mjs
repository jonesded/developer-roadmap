...
import { r as removeAuthToken } from './BaseLayout_B-BXcELP.mjs';
...
function UploadProfilePicture(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (type === "avatar") {
      res = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-upload-profile-picture`,
        {
          method: "POST",
          body: formData,
          credentials: "include"
      ...
      res = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-upload-team-logo/${teamId}`,
        {
          method: "POST",
          body: formData,
          credentials: "include"
