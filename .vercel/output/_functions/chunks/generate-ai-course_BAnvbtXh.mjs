...
import { u as useOutsideClick, i as isLoggedIn, c as cn, r as removeAuthToken, g as getUser, M as Modal } from './BaseLayout_B-BXcELP.mjs';
...
import { q as queryClient, c as httpPatch, a as httpPost } from './query-client_CBXGK_YM.mjs';
import { s as slugify } from './slugger_ChlNJxtz.mjs';
import { g as getAiCourseOptions, a as aiLimitOptions } from './ai-course_C2IOZ4Jw.mjs';
/* empty css                          */
...
import { R as ResizablePanelGroup, a as ResizablePanel, b as ResizableHandle, c as AICourseLessonChat } from './AICourseLessonChat_CBM8swR8.mjs';
...
import { a as renderTopicProgress } from './resource-progress_R1rWsSWL.mjs';
...
function TestMyKnowledgeAction(props) {
  ...
  const generateAiLessonQuestions = async () => {
    ...
    const response = await fetch(
      `${"https://api.imoogleai.xyz"}/v1-generate-ai-course-lesson-question/${courseSlug}`,
      {
        method: "POST",
        headers: {
...
function AICourseLesson(props) {
  ...
  const generateAiCourseContent = async (isForce, customPrompt) => {
    ...
    const response = await fetch(
      `${"https://api.imoogleai.xyz"}/v1-generate-ai-course-lesson/${courseSlug}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
...
function AICourseRoadmapView(props) {
  ...
  const generateAICourseRoadmap = async (courseSlug2) => {
    ...
    try {
      const response = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-generate-ai-course-roadmap/${courseSlug2}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
  ...
  const { mutate: forkCourse } = useMutation(
    {
      mutationFn: async () => {
        setIsPending(true);
        return httpPost(
          `${"https://api.imoogleai.xyz"}/v1-fork-ai-course/${courseSlug}`,
          {}
        );
      },
      onSuccess(data) {
  ...
  try {
    ...
    if (slug && isForce) {
      response = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-regenerate-ai-course/${slug}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
      ...
      response = await fetch(
        `${"https://api.imoogleai.xyz"}/v1-generate-ai-course`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
