...
import { u as useOutsideClick, i as isLoggedIn, c as cn, r as removeAuthToken, g as getUser, M as Modal } from './BaseLayout_BrSKT5L_.mjs';
import { u as useIsPaidUser, b as billingDetailsOptions } from './billing_raRJBpT1.mjs';
...
import { q as queryClient, c as httpPatch, a as httpPost } from './query-client_rwy39LWF.mjs';
import { s as slugify } from './slugger_ChlNJxtz.mjs';
import { g as getAiCourseOptions, a as aiLimitOptions } from './ai-course_sZQdwLbV.mjs';
/* empty css                          */
...
import { R as ResizablePanelGroup, a as ResizablePanel, b as ResizableHandle, c as AICourseLessonChat } from './AICourseLessonChat_GakhU-8L.mjs';
import { g as getPercentage } from './number_D9-I_I5P.mjs';
import { C as CheckIcon$1 } from './CheckIcon_B3c1RSNy.mjs';
import { A as AILimitsPopup } from './AILimitsPopup_C4V0wYi5.mjs';
/* empty css                          */
import { generateAIRoadmapFromText, renderFlowJSON } from './index_DJK1nS1V.mjs';
import { r as replaceChildren } from './dom_C6E8wq7N.mjs';
import { a as renderTopicProgress } from './resource-progress_D7IfDnhr.mjs';
...
function TestMyKnowledgeAction(props) {
  ...
  const generateAiLessonQuestions = async () => {
    ...
    const response = await fetch(
      `${undefined                              }/v1-generate-ai-course-lesson-question/${courseSlug}`,
      {
        method: "POST",
        headers: {
...
function AICourseLesson(props) {
  ...
  const generateAiCourseContent = async (isForce, customPrompt) => {
    ...
    const response = await fetch(
      `${undefined                              }/v1-generate-ai-course-lesson/${courseSlug}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
...
function AICourseRoadmapView(props) {
  ...
  const generateAICourseRoadmap = async (courseSlug2) => {
    ...
    try {
      const response = await fetch(
        `${undefined                              }/v1-generate-ai-course-roadmap/${courseSlug2}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
  ...
  const { mutate: forkCourse } = useMutation(
    {
      mutationFn: async () => {
        setIsPending(true);
        return httpPost(
          `${undefined                              }/v1-fork-ai-course/${courseSlug}`,
          {}
        );
      },
      onSuccess(data) {
  ...
  try {
    ...
    if (slug && isForce) {
      response = await fetch(
        `${undefined                              }/v1-regenerate-ai-course/${slug}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
      ...
      response = await fetch(
        `${undefined                              }/v1-generate-ai-course`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
