...
import { h as httpPost, T as TOKEN_COOKIE_NAME, g as getUser, e as httpGet } from './BaseLayout_BrSKT5L_.mjs';
...
async function loadFreshProgress(resourceType, resourceId) {
  const { response, error } = await httpGet(`${undefined                              }/v1-get-user-resource-progress`, {
    resourceType,
    resourceId
  });
  if (error || !response) {
