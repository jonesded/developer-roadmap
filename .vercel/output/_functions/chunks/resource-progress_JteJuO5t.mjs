...
import { h as httpPost, T as TOKEN_COOKIE_NAME, g as getUser, e as httpGet } from './BaseLayout_DfQviQZ5.mjs';
...
async function updateResourceProgress(topic, progressType) {
  const { topicId, resourceType, resourceId } = topic;
  const { response, error } = await httpPost(`${"https://api.hnmdevs.com"}/v1-update-resource-progress`, {
    topicId,
    resourceType,
    resourceId,
...
async function loadFreshProgress(resourceType, resourceId) {
  const { response, error } = await httpGet(`${"https://api.hnmdevs.com"}/v1-get-user-resource-progress`, {
    resourceType,
    resourceId
  });
  if (error || !response) {
