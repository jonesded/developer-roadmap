...
import { p as pageProgressMessage, e as httpGet, n as httpPatch } from '../../chunks/BaseLayout_BrSKT5L_.mjs';
...
function NotificationPage() {
  ...
  const lostNotifications = async () => {
    const { error: error2, response } = await httpGet(
      `${undefined                              }/v1-get-invitation-list`
    );
    if (error2 || !response) {
  ...
  async function respondInvitation(status, inviteId) {
    ...
    const { response, error: error2 } = await httpPatch(
      `${undefined                              }/v1-respond-invite/${inviteId}`,
      {
        status
      }
    );
