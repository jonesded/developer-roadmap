...
import { c as cn, u as useOutsideClick, p as pageProgressMessage, n as httpPatch, h as httpPost, e as httpGet } from '../../chunks/BaseLayout_BrSKT5L_.mjs';
...
function VisibilityDropdown(props) {
  ...
  async function updateProfileVisibility(visibility2) {
    ...
    const { error } = await httpPatch(
      `${undefined                              }/v1-update-public-profile-visibility`,
      {
...
function ProfileUsername(props) {
  ...
  const checkIsUnique = async (username2) => {
    ...
    setIsLoading(true);
    const { response, error } = await httpPost(`${undefined                              }/v1-check-is-unique-username`, {
      username: username2
    });
    if (error || !response) {
      setIsUnique(null);
...
function UpdatePublicProfileForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, error } = await httpPatch(
      `${undefined                              }/v1-update-public-profile-config`,
      {
        isAvailableForHire,
        isEmailVisible,
        profileVisibility,
  ...
  const loadProfileSettings = async () => {
    ...
    const { error, response } = await httpGet(
      `${undefined                              }/v1-get-profile-settings`
    );
    if (error || !response) {
      setIsLoading(false);
      toast.error(error?.message || "Something went wrong");
  ...
  const loadProfileRoadmaps = async () => {
    setIsLoading(true);
    const { error, response } = await httpGet(`${undefined                              }/v1-get-profile-roadmaps`);
    if (error || !response) {
      setIsLoading(false);
      toast.error(error?.message || "Something went wrong");
      return;
