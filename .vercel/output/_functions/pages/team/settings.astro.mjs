...
import { u as useOutsideClick, l as useKeydown, v as httpDelete, p as pageProgressMessage, S as Spinner, e as httpGet, w as httpPut } from '../../chunks/BaseLayout_B-BXcELP.mjs';
...
function DeleteTeamPopup(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, error: error2 } = await httpDelete(
      `${"https://api.imoogleai.xyz"}/v1-delete-team/${teamId}`
    );
    if (error2 || !response) {
      setIsLoading(false);
...
function UpdateTeamForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, error } = await httpPut(
      `${"https://api.imoogleai.xyz"}/v1-update-team/${teamId}`,
      {
        name,
        website,
        type: teamType,
  ...
  async function loadTeam() {
    const { response, error } = await httpGet(
      `${"https://api.imoogleai.xyz"}/v1-get-team/${teamId}`
    );
    if (error || !response) {
      console.log(error);
      return;
